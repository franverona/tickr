...
import type { PgDbSchema, PgTasksTable } from './schema'
...
async function getTaskTagIds(db: Kysely<PgDbSchema>, taskId: string): Promise<string[]> {
  const rows = await db
...
async function getTaskUrls(db: Kysely<PgDbSchema>, taskId: string): Promise<TaskUrl[]> {
  return db
...
async function buildTagMap(db: Kysely<PgDbSchema>): Promise<Map<string, string[]>> {
  const rows = await db
    .selectFrom('task_tags')
...
async function buildUrlMap(db: Kysely<PgDbSchema>): Promise<Map<string, TaskUrl[]>> {
  const rows = await db
    .selectFrom('task_urls')
...
function rowToTask(row: PgTasksTable, tags: string[], urls: TaskUrl[]): Task {
  return {
    id: row.id,
    title: row.title,
...
export class PostgresTaskRepository implements TaskRepository {
  constructor(private readonly kysely: Kysely<PgDbSchema>) {}

  private async ready(): Promise<Kysely<PgDbSchema>> {
  ...
  async createTask(data: {
    ...
    const minRow = await sql<{ min: number | null }>`
      SELECT MIN(sort_order) AS min FROM tasks WHERE completed = false
    `.execute(db)
  ...
  async updateTask(
    ...
    const next = {
      ...
      completed: data.completed !== undefined ? data.completed : current.completed,
      archived: data.archived !== undefined ? data.archived : current.archived,
